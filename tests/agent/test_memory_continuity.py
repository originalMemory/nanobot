...
def test_duplicate_files_cannot_fill_recall_or_qualify_topic(tmp_path):
    ...
    for i in range(25):
        (tmp_path / f"2026-01-01 周一.sync-conflict-{i}.md").write_text(Path(original).read_text())
    result = memory._search_diary("星海", str(tmp_path))
    assert len(result.hits) == 1
    assert result.topic is None
    assert len(_grep_diary(tmp_path, "星海", None, None, 10)) == 1
...
def test_body_ignores_weather_and_empty_summary(tmp_path):
    ...
    assert memory._extract_summary(Path(filename).read_text()) == ""
    assert memory._search_diary("星海", str(tmp_path)).hits == []
    assert "天气数据" not in diary_body(Path(filename).read_text())
...
def test_fingerprint_ignores_weather_and_absolute_root(tmp_path):
    ...
    Path(b).write_text(Path(b).read_text() + "\n# 天气\n温度: 30\n")
    assert fp == memory._topic_fingerprint([b], ("星海",), str(second))
    Path(b).write_text(Path(b).read_text().replace("登场", "离开"))
    assert fp != memory._topic_fingerprint([b], ("星海",), str(second))
...
async def test_build_card_validates_quote_and_preserves_unknown_fields(tmp_path):
    ...
    saved = json.loads(memory._topic_path(topics, "星海").read_text())
...
async def test_source_change_during_generation_does_not_replace(tmp_path):
    ...
    assert not await memory._build_topic_card(topic="星海", files=[filename], fingerprint=fp,
                                              topic_dir=topics, summarize=model, diary_root=str(tmp_path))
    assert json.loads(memory._topic_path(topics, "星海").read_text())["summary"] == "旧摘要"
...
async def test_generation_failure_single_flight_and_persistent_cooldown(tmp_path):
    ...
    logs = [json.loads(line) for line in hook._log_path.read_text().splitlines()]
...
async def test_rejection_rechecks_only_changed_evidence(tmp_path):
    ...
    Path(filename).write_text(Path(filename).read_text() + "\n星海后续新增阶段。")
...
async def test_invalid_model_output_keeps_previous_card(tmp_path):
    ...
    assert json.loads(memory._topic_path(topics, "星海").read_text()) == old
...
def test_prompt_contracts_keep_dates_actors_and_transitions():
    archive = render_template("agent/consolidator_archive.md")
    dream = render_template("agent/dream.md", skill_creator_path="skills/creator")
    assert "less than two weeks" in archive
    assert "original message timestamp" in archive
    assert "Do not turn the assistant's suggestion" in archive
...
async def test_canonical_rename_cannot_overwrite_another_card(tmp_path):
    ...
    assert not await memory._build_topic_card(topic="Star Ocean", files=[filename], fingerprint=fp,
                                              topic_dir=topics, summarize=model, diary_root=str(tmp_path))
    assert json.loads(memory._topic_path(topics, "星海").read_text())["custom"] == "keep"
...
async def test_missing_source_never_replaces_previous_card(tmp_path):
    ...
    assert json.loads(memory._topic_path(topics, "星海").read_text()) == original
...
def test_conflict_removal_is_visible_in_recall_log_data(tmp_path):
    filename = note(tmp_path)
    (tmp_path / '2026-01-01 周一.sync-conflict-copy.md').write_text(Path(filename).read_text())
    result = memory._search_diary('星海', str(tmp_path))
...
async def test_review_attaches_verified_name_without_new_card(tmp_path, action, candidate, body):
    ...
    updated = json.loads(memory._topic_path(hook._topic_dir, '星海').read_text())
    ...
    cache = json.loads(memory._topic_decision_path(hook._topic_dir, candidate).read_text())
...
async def test_failed_legacy_upgrade_preserves_old_card_and_does_not_cache_link(tmp_path):
    ...
    await hook.on_finally(AgentRunHookContext(messages=[]))
    await scheduled.pop()
    assert json.loads(memory._topic_path(hook._topic_dir,'星海').read_text())==original
...
def test_manual_search_filters_metadata_before_limit(tmp_path):
    note(tmp_path,date='2026-01-01',body='去北京见了老朋友。')
    for i in range(2,13):
        (tmp_path/f'2026-01-{i:02d}.md').write_text('---\n天气: 北京晴天\n---\n\n在家看电影。')
    hits=_grep_diary(tmp_path,'北京',None,None,10)
    assert len(hits)==1 and hits[0]['date']=='2026-01-01'


def test_manual_search_fills_or_after_date_filter(tmp_path):
    note(tmp_path,date='2026-01-01',body='只聊了北京。')
    for i in range(2,13):
        note(tmp_path,date=f'2026-01-{i:02d}',body='北京的朋友。')
    hits=_grep_diary(tmp_path,'北京 朋友',None,'2026-01-01',1)
    assert len(hits)==1 and hits[0]['match_type']=='or'
    assert _grep_diary(tmp_path,'北京',None,None,0)==[]
