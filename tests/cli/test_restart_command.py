...
class TestRestartCommand:
    ...
    async def test_run_agent_loop_estimates_usage_when_provider_omits_it(self, monkeypatch):
        ...
        await loop._run_agent_loop([], runtime=loop.llm_runtime())
        assert loop._last_usage["turn_prompt_tokens"] == 123
        assert loop._last_usage["turn_completion_tokens"] == 7
        assert loop._last_usage["last_prompt_tokens"] == 123
        assert loop._last_usage["last_completion_tokens"] == 7
