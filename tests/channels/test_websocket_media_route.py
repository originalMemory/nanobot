...
``channel._http_router.sign_media_path`` mints URLs during session reads,
and ``ForkGatewayHTTPHandler._handle_media_fetch`` serves the bytes back.
...
def test_sign_media_path_rejects_paths_outside_media_root(
    ...
    with patch("nanobot.webui.fork_http.get_media_dir", return_value=media):
        assert channel._http_router.sign_media_path(outside) is None
        # Traversal via the media root is also rejected — the resolve() step
        # normalises ``..`` out before the relative_to check.
        assert channel._http_router.sign_media_path(media / ".." / "secrets" / "cred.txt") is None
...
def test_sign_media_path_round_trips_via_hmac(
    ...
    channel = _ch(bus, port=0)
    with patch("nanobot.webui.fork_http.get_media_dir", return_value=media):
        url = channel._http_router.sign_media_path(media / "a.png")
    assert url is not None
    assert url.startswith("/api/media/")
    sig, payload = url[len("/api/media/"):].split("/", 1)
    expected = hmac.new(
        channel._http_router._media_secret, payload.encode("ascii"), hashlib.sha256
    ).digest()[:16]
...
def _patch_media_dir(media: Path):
    """Patch both locations that call get_media_dir so staging and signing use tmp_path."""
    from contextlib import ExitStack
    stack = ExitStack()
    fake = _fake_media_dir(media)
    stack.enter_context(patch("nanobot.webui.fork_http.get_media_dir", side_effect=fake))
    stack.enter_context(patch("nanobot.utils.media_staging.get_media_dir", side_effect=fake))
    return stack
...
def test_local_markdown_image_is_staged_and_rewritten(
    ...
    with _patch_media_dir(media):
        rewritten = channel._http_router.rewrite_local_markdown_images(
...
def test_local_markdown_image_rejects_workspace_escape(
    ...
    with _patch_media_dir(media):
        assert channel._http_router.rewrite_local_markdown_images(text) == text
...
async def test_media_route_serves_signed_file(
    ...
    with patch("nanobot.webui.fork_http.get_media_dir", return_value=media):
        url_path = channel._http_router.sign_media_path(target)
...
async def test_media_route_rejects_bad_signature(
    ...
    with patch("nanobot.webui.fork_http.get_media_dir", return_value=media):
        good = channel._http_router.sign_media_path(media / "f.png")
