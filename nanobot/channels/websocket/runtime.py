...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(
        ...
        *,
        ...
        self._screenshot_futures: dict[str, asyncio.Future[Path | None]] = {}
    ...
    def _cleanup_connection(self, connection: Any) -> None:
        ...
        for request_id in self._conn_screenshot_requests.pop(connection, set()):
    ...
    async def _connection_loop(self, connection: Any) -> None:
        ...
        requested_chat_id = _query_first(query, "chat_id")
        default_chat_id = (
            requested_chat_id
            if _is_valid_chat_id(requested_chat_id)
            else str(uuid.uuid4())
        )
    ...
    async def _dispatch_envelope(
        ...
        if t == "presence":
            ...
            self._conn_focused[connection] = focused if isinstance(focused, bool) else True
            locked = envelope.get("locked")
            if isinstance(locked, bool):
                self._conn_locked[connection] = locked
            return
        if t == "screenshot_result":
            await self._handle_screenshot_result(envelope)
            return
        if t == "new_chat":
        ...
        if t == "attach":
            ...
            if not _is_valid_chat_id(cid):
                ...
                return
            if self.gateway.session_manager is not None and cid != INBOX_UNIFIED_CHAT_ID:
                self.gateway.session_manager.get_or_create(f"websocket:{cid}")
    ...
    async def stop(self) -> None:
        ...
        self._last_user_conn = None
        for future in self._screenshot_futures.values():
            if not future.done():
                future.cancel()
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        payload: dict[str, Any] = {
            "event": "user" if msg.metadata.get("_unified_inbox_inbound") else "message",
            "chat_id": msg.chat_id,
            "text": wire_text,
        }
        if msg.media:
        ...
        source_channel = msg.metadata.get("source_channel")
        if isinstance(source_channel, str) and source_channel:
            payload["source_channel"] = source_channel
        source_chat_id = msg.metadata.get("source_chat_id")
        ...
        if self._unified_session and msg.chat_id != INBOX_UNIFIED_CHAT_ID:
            await self.fan_out_unified_inbox_event(payload, "websocket", msg.chat_id)
    ...
    async def fan_out_unified_inbox_event(
        ...
        for connection in list(self._subs.get(INBOX_UNIFIED_CHAT_ID, ())):
            await self._safe_send_to(connection, raw, label=" unified-inbox ")

    async def _handle_screenshot_result(self, envelope: dict[str, Any]) -> None:
        request_id = envelope.get("request_id")
        data = envelope.get("data")
        if not isinstance(request_id, str) or not isinstance(data, str):
            return
        ...
        if len(encoded) * 3 // 4 > _MAX_SCREENSHOT_BYTES:
            future.set_result(None)
            return
        try:
            raw = base64.b64decode(encoded, validate=True)
            target = get_media_dir("websocket") / "screenshots" / f"{request_id}.jpg"
    ...
    async def request_screenshot(
        ...
        *,
        ...
        future: asyncio.Future[Path | None] = asyncio.get_running_loop().create_future()
        ...
        try:
            ...
            requests = self._conn_screenshot_requests.get(connection)
            if requests is not None:
                requests.discard(request_id)
                if not requests:
                    self._conn_screenshot_requests.pop(connection, None)
    ...
    def get_unfocused_last_user_connection(self) -> tuple[Any, str] | None:
        connection = self._last_user_conn
        if connection is None or connection not in self._conn_default:
            return None
        if self.is_connection_focused(connection) or self.is_connection_locked(connection):
            return None
        return connection, self._conn_default[connection]
    ...
    async def send_delta(
        ...
        *,
        ...
        if stream_id is None:
            raw_stream_id = meta.get("_stream_id")
        ...
        if self._unified_session and chat_id != INBOX_UNIFIED_CHAT_ID:
            await self.fan_out_unified_inbox_event(body, "websocket", chat_id)
