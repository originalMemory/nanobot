...
class Consolidator:
    ...
    def _eager_trim_consolidated_prefix(self, session: Session, end_idx: int) -> None:
        """归档并物理移除已整合的消息前缀。"""
        if end_idx <= 0 or end_idx > len(session.messages):
            return
        trimmed = list(session.messages[:end_idx])
        if self.history_store is not None:
            self.history_store.insert_messages(session.key, trimmed, "consolidation")
    ...
    async def compact_idle_session(
        ...
        *,
        ...
        async with lock:
            ...
            trimmed_messages = list(session.messages[:session.last_consolidated])
            trimmed_messages.extend(messages_to_remove)
            if self.history_store is not None and trimmed_messages:
