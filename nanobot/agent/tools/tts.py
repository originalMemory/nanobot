...
@tool_parameters(tool_parameters_schema(
    text=StringSchema("要朗读的文本。中日混合时用 [zh]...[/zh] 和 [ja]...[/ja] 分段；标签不写入回复正文。", min_length=1, max_length=10000),
    required=["text"],
))
class TtsTool(Tool):
    _plugin_discoverable = False

    def __init__(self, speech: SpeechService):
        self.speech = speech
    ...
    async def execute(self, text: str = "", **_: Any) -> str:
        ...
        try:
            audio = self.speech.submit(request.chat_id, turn_id, text, channel=request.channel,
                               metadata=request.metadata, session_key=request.session_key)
        except ValueError as exc:
            return f"Error: {exc}"
        request.attributes["speech"] = audio
