...
class ActiveMemoryHook(AgentHook):
    ...
    def __init__(
        ...
        *, topic_lock: asyncio.Lock | None = None, topic_tasks: set[str] | None = None,
        ...
        if workspace and self._topic_dir and not self._topic_dir.resolve().is_relative_to(Path(workspace).resolve()):
            raise ValueError("主题卡目录必须位于工作区内")
        self._summarize: Callable[[str], Awaitable[str]] | None = None
        self._schedule: Callable[[Coroutine[Any, Any, None]], None] | None = None
        self._topic_tasks = topic_tasks if topic_tasks is not None else set[str]()
        self._topic_lock = topic_lock if topic_lock is not None else asyncio.Lock()
        self._pending_topic_cards: dict[str, tuple[str, list[str], str, str]] = {}
        self._injected_message: dict[str, Any] | None = None
        self._original_content: Any = None
    ...
    async def before_iteration(self, context: AgentHookContext) -> None:
        ...
        # 取最新的用户消息
        user_msg = context.messages[-1] if context.messages else None
        ...
        self._injected_message = user_msg
        self._original_content = user_msg.get("content")
    ...
    async def _extract_keywords(self, text: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=OLLAMA_TIMEOUT, trust_env=False) as client:
            ...
            value = cast(dict[str, Any], message).get("content") if isinstance(message, dict) else None
            if not isinstance(value, str):
                raise ValueError("invalid_keyword_response")
            return value.strip().replace("\n", " ")

    async def on_finally(self, context: AgentRunHookContext) -> None:
        """当前回复结束后再启动摘要，避免与用户请求争用主模型。"""
        if self._injected_message is not None:
            self._injected_message["content"] = self._original_content
            self._injected_message = None
        pending = list(self._pending_topic_cards.values())
        self._pending_topic_cards.clear()
        if context.exception is not None or context.error:
            return
    ...
    def _schedule_topic_card(
        ...
        if not self._topic_dir or not self._summarize or not self._schedule:
            return
        key = _topic_key(topic)
        if key in self._topic_tasks:
            return
        self._topic_tasks.add(key)

        async def build() -> None:
            assert self._topic_dir is not None and self._summarize is not None
...
def create_active_memory_hook_factory(agent: AgentLoop, config: Config) -> AgentTurnHookFactory | None:
    ...
    def factory(turn: AgentTurnHookContext) -> AgentHook | None:
        ...
        async def summarize(prompt: str) -> str:
            # 后台请求不继承用户轮次的路由上下文，也不注册工具。
            with request_context(RequestContext(channel="system", chat_id="active-memory")), llm_usage_source("system"):
                response = await asyncio.wait_for(runtime.provider.chat_with_retry(
                    messages=[{"role": "user", "content": prompt}], tools=None,
                    model=runtime.model, temperature=0.1,
                    max_tokens=runtime.generation.max_tokens,
                    reasoning_effort=runtime.generation.reasoning_effort,
                ), timeout=120)
            if response.finish_reason == "error":
                raise RuntimeError("active_memory_topic_summary_failed")
            return response.content or ""

        hook.configure_topic_summary(summarize, agent.schedule_background)
        return hook
...
def _search_diary(
    ...
    try:
        ...
        for path in sorted(set[str]().union(*raw_sets)):
        ...
        excluded = len(set[str]().union(*raw_sets) - set[str]().union(*sets))
...
def _topic_index(topic_dir: Path) -> list[dict[str, Any]]:
    ...
    return sorted([{
        "topic": card["topic"],
        "aliases": sorted(set(_strings(card.get("aliases")))),
        "entities": sorted({e["name"] for e in _objects(card.get("related_entities"))
                            if e.get("name") and e.get("sources")
...
def _load_topic_cards(topic_dir: Path | None) -> list[dict[str, Any]]:
    ...
    for path in topic_dir.glob("*.json"):
        ...
        if isinstance(card, dict):
            ...
            if isinstance(card.get("topic"), str) and card["topic"]:
                cards.append(card)
    return cards
...
def _find_topic_card(
    ...
    for card in _load_topic_cards(topic_dir):
        ...
        alias = any(found(str(name)) for name in _strings(card.get("aliases")))
        entities = [e for e in _objects(card.get("related_entities"))
                    if found(str(e.get("name", "")))
                    and e.get("sources")]
...
def _topic_names(card: dict[str, Any]) -> set[str]:
    return {
        ...
        *(str(alias).strip() for alias in _strings(card.get("aliases"))),
    } - {""}
...
async def _assess_topic_candidate(
    *, topic: str, files: list[str], summarize: Callable[[str], Awaitable[str]],
    ...
    if action in {"alias", "entity"}:
        ...
        sources: set[str] = set()
        for ref in _objects(decision.get("evidence")):
...
def _attach_topic_candidate(topic_dir: Path, topic: str, decision: dict[str, Any]) -> None:
    ...
    if decision["action"] == "alias":
        card["aliases"] = sorted(set(_strings(card.get("aliases"))) | {topic})
    else:
...
async def _build_topic_card(
    *, topic: str, files: list[str], fingerprint: str, topic_dir: Path,
    ...
    kept_aliases = set(_strings(confirmed["aliases"]))
    entities: dict[str, dict[str, Any]] = {
        e["name"].casefold(): {"name": e["name"], "relation": e["relation"],
                             "sources": list(e["sources"])}
        for e in _objects(confirmed["related_entities"])
        if e.get("name") and e.get("relation") and e.get("sources")
    }
    removed_mappings: set[tuple[str, str]] = set()
    for removal in _objects(generated.get("mapping_removals")):
        if not isinstance(removal.get("name"), str):
            continue
        ...
        if (kind not in {"alias", "entity"} or not isinstance(removal.get("reason"), str)
            ...
            continue
        verified = False
        for ref in _objects(removal.get("evidence")):
            path, quote = ref.get("path"), ref.get("quote")
    ...
    aliases = sorted((kept_aliases | {
        alias.strip() for alias in _strings(generated.get("aliases"))
        if contains_diary_term(source, alias.strip())
    ...
    for entity in _objects(generated.get("related_entities")):
        if entity.get("exclusive") is not True:
            continue
        ...
        for item in _objects(entity.get("evidence")):
...
def _format_injection(
    ...
    if topic_card and topic_card.get("summary"):
        ...
        matched_entities = _objects(topic_card.get("_matched_related_entities"))
        if matched_entities:
            lines.append("当前关联：")
...
def _extract_text(content: Any) -> str:
    ...
    if isinstance(content, str):
        ...
        parts = [str(p.get("text", "")) for p in _objects(content)]
...
def _validate_keywords(raw: str, text: str, topic_dir: Path | None = None) -> tuple[str, list[str]]:
    ...
    for word in [*raw.split(), *numeric]:
        ...
        invalid = (
            len(word) > 80
            or key in {"第", "张", "个", "这个", "那个", "这些", "那些", "美照", "照片", "图片", "买菜"}
