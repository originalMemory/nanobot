...
def update_agent_settings(query: QueryParams) -> dict[str, Any]:
    config = load_config()
    defaults = config.agents.defaults
    generation_target = config.resolve_preset()
    changed = False
    ...
    if vision_model is not None:
        value = vision_model.strip() or None
        if generation_target.vision_model != value:
            generation_target.vision_model = value
            changed = True

    vision_provider = _query_first_alias(query, "vision_provider", "visionProvider")
    if vision_provider is not None:
        value = vision_provider.strip() or None
        if generation_target.vision_provider != value:
            generation_target.vision_provider = value
            changed = True
...
def update_tts_settings(query: QueryParams) -> dict[str, Any]:
    ...
    if default_voice is not None:
        value = default_voice.strip()
        if len(value) > 200:
            raise WebUISettingsError("default_voice is too long")
        if tts_config.default_voice != value:
            tts_config.default_voice = value
            changed = True
