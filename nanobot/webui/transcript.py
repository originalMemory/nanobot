...
def _session_media_parts(media: Any) -> tuple[list[str], list[dict[str, str]]]:
    local: list[str] = []
...
def session_messages_to_wire_events(
    ...
    """将统一 Session 历史转换为官方 WebUI transcript 事件。"""
    ...
    def flush_next_user_delivery_event(tool_event: dict[str, Any]) -> None:
        if not pending_user_delivery_events:
            return
        match_index = 0
        arguments = tool_event.get("arguments")
        if isinstance(arguments, dict):
            for index, event in enumerate(pending_user_delivery_events):
                if (
                    (not arguments.get("channel") or event.get("source_channel") == arguments.get("channel"))
                    and (
                        not arguments.get("chat_id")
                        or event.get("source_chat_id") == arguments.get("chat_id")
                    )
                    and (
                        not isinstance(arguments.get("content"), str)
                        or event.get("text") == arguments.get("content")
                    )
                ):
                    match_index = index
                    break
    ...
    for message in messages:
        ...
        for key in ("source_channel", "source_chat_id", "timestamp"):
        ...
        cron_job_name = message.get("_cron_job_name")
        if isinstance(cron_job_name, str) and cron_job_name:
            base["cron_job_name"] = cron_job_name

        local_media, remote_media = _session_media_parts(message.get("media"))
        if role == "user":
            flush_user_delivery_events()
            event: dict[str, Any] = {**base, "event": "user", "text": text}
            if local_media:
                event["media_paths"] = local_media
            if remote_media:
                event["media_urls"] = remote_media
            events.append(event)
            continue
        ...
        if role == "assistant":
            ...
            if isinstance(reasoning, str) and reasoning:
                events.append({**base, "event": "reasoning_delta", "text": reasoning})
                events.append({**base, "event": "reasoning_end"})
            ...
            if isinstance(tool_calls, list):
                for call in tool_calls:
                    ...
                    pending_tools[str(call.get("id") or "")] = {
                        "name": str(function.get("name") or ""),
                        "arguments": arguments,
                    }

            if text or local_media or remote_media:
                event = {**base, "event": "message", "text": text}
                if local_media:
                    event["media"] = local_media
                if remote_media:
                    event["media_urls"] = remote_media
                latency = message.get("latency_ms")
                if isinstance(latency, (int, float)):
                    event["latency_ms"] = int(latency)
                usage = message.get("usage")
                if isinstance(usage, dict) and usage:
                    event["usage"] = usage
                if message.get("_channel_delivery"):
                    event["channel_delivery"] = True
                if message.get("_user_initiated_channel_delivery"):
                    event["user_initiated_delivery"] = True
                if event.get("channel_delivery") and event.get("user_initiated_delivery"):
                    pending_user_delivery_events.append(event)
                    continue
                events.append(event)
            continue
        ...
        if role == "tool":
            ...
            pending = pending_tools.pop(call_id, {})
            tool_event: dict[str, Any] = {
                "version": 1,
                "phase": "end",
                "call_id": call_id,
                "name": pending.get("name") or str(message.get("name") or ""),
                "result": text,
            }
            if pending.get("arguments") is not None:
                tool_event["arguments"] = pending["arguments"]
            events.append({
                **base,
                "event": "message",
                "text": "",
            ...
            if tool_event["name"] == "message":
                flush_next_user_delivery_event(tool_event)
    ...
    *,
    ...
    for idx, rec in enumerate(lines):
        ...
        if ev == "user":
            ...
            remote_media = _media_from_signed_urls(rec.get("media_urls"))
            if remote_media:
                media_att = [*(media_att or []), *remote_media]
            row: dict[str, Any] = {
                "id": _new_id("u", idx),
                "role": "user",
                "content": text_s,
            ...
            if isinstance(source_channel, str) and source_channel:
                row["sourceChannel"] = source_channel
            source_chat_id = rec.get("source_chat_id")
            if isinstance(source_chat_id, str) and source_chat_id:
                row["sourceChatId"] = source_chat_id
        ...
        if ev == "message":
            ...
            source_channel = rec.get("source_channel")
            if isinstance(source_channel, str) and source_channel:
                extra["sourceChannel"] = source_channel
            source_chat_id = rec.get("source_chat_id")
            if isinstance(source_chat_id, str) and source_chat_id:
                extra["sourceChatId"] = source_chat_id
            if rec.get("channel_delivery"):
                extra["channelDelivery"] = True
            if rec.get("user_initiated_delivery"):
                extra["userInitiatedDelivery"] = True
            cron_job_id = rec.get("cron_job_id")
            if isinstance(cron_job_id, str) and cron_job_id:
                extra["cronJobId"] = cron_job_id
            cron_job_name = rec.get("cron_job_name")
            if isinstance(cron_job_name, str) and cron_job_name:
                extra["cronJobName"] = cron_job_name
