...
- [x] 3.4 建立六册 PDF manifest：记录本地路径、书册/级别、页数、大小、修改时间和 SHA-256；检测文本层并确定渲染策略，不把 PDF 或绝对路径写入仓库产物。
- [x] 3.5 实现 `extract_curriculum.py`：Poppler 逐页渲染、本地 Ollama `qwen3.8:27b` vision 严格 JSON 输出、默认单并发、原子页结果、输入/prompt/schema/model 版本指纹、pending/completed/failed 状态，以及按书册、页码范围、失败页和强制页断点续跑。
- [x] 3.6 用初级上 PDF 第 37 页单元总览及第 38～47 页第 1 课做试点；人工核对页面类型、课号续页、句型、语用限制、词汇主题和练习类型，并固定 prompt/schema。
- [x] 3.7 运行并课程级合并初级上下候选结果，整理和交叉核验 48 个初级课程节点。
- [x] 3.8 运行并课程级合并中级上下候选结果，整理和交叉核验 32 个中级课程节点，包含交际功能和篇章技能。
- [x] 3.9 运行并课程级合并高级上下候选结果，整理和交叉核验 24 个高级课程节点，包含语域、隐含态度、篇章结构和表达辨析。
- [x] 3.10 增加 Foundation→N1 前置关系、桥接节点和 JLPT 官方题型覆盖元数据；所有 Qwen 初次输出保持 candidate，复核后才升级。
- [x] 3.11 校验恰好 104 个教材 ID、唯一节点 ID、可解析前置引用、允许的验证状态、来源引用、来源页码和无循环依赖，并扫描仓库确保没有页级 OCR/视觉中间产物。
...
- [x] 4.5 实现下一节点排序：前置关系、映射后的 Anki 弱项、教材课序、题型缺口、学习状态和桥接节点；定义 Anki 不可用或没有 verified 可教节点时的行为。
- [x] 4.6 实现约 5、15～20、30 分钟的微型、标准、深度课程计划；不得为凑复习数量加入未到期卡。
- [x] 4.7 实现 `new → learning → reviewing → mastered`，要求至少两个不同会话 ID 下的识别与主动输出证据后才 mastered。
- [x] 4.8 增加 planner/state 测试：能力偏科、0～2 个到期卡、缺少前置、桥接返回、Anki 不可用、无可教节点、疲劳降级、mastery 降级、并发写和阶段 gate。
