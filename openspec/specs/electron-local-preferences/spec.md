...
Electron 应用 SHALL 将用户的主题偏好（`light` | `dark`）持久化到 `electron-store` 的 `appearance.theme` 键下。主题变更 SHALL 通过 `electronAPI.config.set("appearance.theme", value)` 写入，并在启动时通过 `electronAPI.config.get("appearance.theme")` 读取。

#### Scenario: 主题变更在重启后保持
- **WHEN** 用户将主题切换为深色模式
- **THEN** `appearance.theme` 在 electron-store 中被设置为 `"dark"`
- **AND** 下次应用启动时，无需用户操作即以深色模式渲染
