...
可选择单个本机场景包，或选择包含多组场景包的父目录；多组目录会读取各自 `manifest.json` 的 `displayName`，通过下拉列表选择实际播放的场景，并可手动刷新列表。内嵌面板和独立窗口都可在标题栏即时切换视频组和轮切模式；轮切支持手动、顺序、随机，在设置中以小时指定间隔，手动选组后重新计时。场景包包含 idle/sunrise、idle/day、idle/sunset、idle/night，以及对应 working 目录；开始时间可在设置中修改。`manifest.json` 使用 `actionLabels` 将不含扩展名的文件名映射为标题动作，例如 `{"actionLabels":{"breathing":"自然呼吸"}}`。缺少或损坏素材时回退到内置视频。素材只读，界面不接收任意文件路径；隐藏/收起面板后卸载视频。
