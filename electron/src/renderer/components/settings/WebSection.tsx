...
interface WebSectionProps {
  ...
  onSave: (update: WebSearchSettingsUpdate) => Promise<void>;
}
...
export function WebSection({
  ...
  const [form, setForm] = useState<WebSearchSettingsUpdate>({
  ...
  const selectedProvider =
    settings.web_search.providers.find((p) => p.name === form.provider) ??
  ...
  const handleProviderChange = (provider: string) => {
    setForm({
      ...
      maxResults: form.maxResults ?? settings.web_search.max_results,
      timeout: form.timeout ?? settings.web_search.timeout,
      useJinaReader: form.useJinaReader ?? settings.web.fetch.use_jina_reader,
    });
    setKeyVisible(false);
    setKeyEditing(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const update: WebSearchSettingsUpdate = {
        ...
        useJinaReader: effectiveJinaReader,
      };
      if (selectedProvider?.credential === "api_key" && form.apiKey?.trim()) {
        update.apiKey = form.apiKey.trim();
      }
      if (selectedProvider?.credential === "base_url") {
        update.baseUrl = form.baseUrl?.trim() ?? "";
      }
      await onSave(update);
      setKeyVisible(false);
      setKeyEditing(false);
      setForm((prev) => ({
        ...prev,
        apiKey: "",
        baseUrl: settings.web_search.base_url ?? prev.baseUrl ?? "",
      }));
    } finally {
      setSaving(false);
    }
              ...
              <div className="relative w-[280px] max-w-full">
                {showKeyInput ? (
                  <>
                    ...
                    >
                      {keyVisible ? <EyeOff className="h-3.5 w-3.5" aria-hidden /> : <Eye className="h-3.5 w-3.5" aria-hidden />}
                    </Button>
                  </>
                ) : (
                  <>
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      onClick={() => { setKeyEditing(true); setKeyVisible(false); }}
        ...
        <SettingsGroup>
          ...
          >
            <NumberInput
              ...
              min={1}
              max={10}
              onChange={(maxResults) => setForm((prev) => ({ ...prev, maxResults }))}
            />
          </SettingsRow>
          <SettingsRow
          ...
          >
            <ToggleButton
              ...
              onChange={(useJinaReader) => setForm((prev) => ({ ...prev, useJinaReader }))}
