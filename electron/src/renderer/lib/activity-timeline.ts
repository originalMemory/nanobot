...
function normalizeCompletedTurnUnits(
  ...
  if (turnUnits.at(-1)?.type !== "activity") return turnUnits;
  ...
  if (!previous || previous.type !== "message" || previous.message.role !== "assistant") {
    return turnUnits;
  }
  return [
...
function visibleMessagesForTurn(messages: UIMessage[]): UIMessage[] {
  return messages
    .filter((message) => !isAgentActivityMember(message))
    .map((message) => assistantHasInlineReasoning(message) ? stripInlineReasoning(message) : message);
}
...
function pushActivityUnits(
  ...
  for (const message of activityMessages) {
    const bucket = message.kind === "trace" && message.fileEdits?.length ? "file" : "other";
    ...
    if ((runBucket && bucket !== runBucket) || segmentChanged) flushRun();
    runBucket = bucket;
...
function reasoningOnlyMessageFromAnswer(message: UIMessage): UIMessage {
  return {
    ...
    isStreaming: !!(message.isStreaming || message.reasoningStreaming),
...
function activityTurnLatencyMs(
  activityMessages: UIMessage[],
  visibleMessages: UIMessage[],
): number | undefined {
  for (let i = visibleMessages.length - 1; i >= 0; i -= 1) {
    if (isValidLatency(visibleMessages[i].latencyMs)) return visibleMessages[i].latencyMs;
  }
  for (let i = activityMessages.length - 1; i >= 0; i -= 1) {
    if (isValidLatency(activityMessages[i].latencyMs)) return activityMessages[i].latencyMs;
  }
  return undefined;
}
