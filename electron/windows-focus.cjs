...
function nativeWindowHandleValue(handle) {
  if (handle.length >= 8) return handle.readBigUInt64LE(0).toString();
  if (handle.length >= 4) return handle.readUInt32LE(0).toString();
  return null;
}

async function captureWindowsForegroundWindow(excludedHandle) {
...
async function restoreWindowsForegroundWindow(handle) {
