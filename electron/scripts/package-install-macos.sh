...
ARCH="${NANOBOT_MAC_ARCH:-$(node -p 'process.arch')}"
...
echo "[2/6] 校验 bundle ID、可执行文件和签名"
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$BUILT_APP/Contents/Info.plist")" == "ai.nanobot.desktop" ]]
[[ -x "$BUILT_APP/Contents/MacOS/nanobot" ]]
