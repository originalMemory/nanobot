...
class CurriculumStateTest(unittest.TestCase):
    ...
    def test_plan_auto_loads_anki_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            self.assertEqual(self.run_script(workspace, "init").returncode, 0)
            ...
            self.assertTrue(plan["anki"]["available"])
            self.assertIn(plan["anki"]["reason"], (None, "no_due_cards"))
    ...
    def test_session_plan_exposes_textbook_route(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            self.assertEqual(self.run_script(workspace, "init").returncode, 0)
            state_path = workspace / "memory" / "japanese-learning-state.json"
            state = json.loads(state_path.read_text(encoding="utf-8"))
            state["nodes"] = {
                "foundation-kana": {"status": "mastered", "evidence": []},
                "foundation-basic-sentence": {"status": "mastered", "evidence": []},
                "textbook-beginner-01": {"status": "learning", "evidence": []},
            }
            state["current_node"] = "textbook-beginner-01"
            state["next_recommendation"] = "textbook-beginner-01"
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            plan = json.loads(result.stdout)
            self.assertEqual(plan["new_target"]["id"], "textbook-beginner-01")
            self.assertEqual(plan["textbook_route"]["authority"], "data/curriculum-n1.yaml")
            self.assertEqual(plan["textbook_route"]["lesson"], 1)
            self.assertIn("beginner-01..48", plan["textbook_route"]["order"])
            self.assertEqual(plan["textbook_route"]["session_scope"], "full_lesson_by_default")
            vocabulary_step = next(
                step for step in plan["steps"] if step["kind"] == "textbook_vocabulary_reference"
            )
            self.assertEqual(vocabulary_step["scope"], "reference_full_lesson_vocabulary")
            self.assertIn("Anki客户端学习", vocabulary_step["rule"])
