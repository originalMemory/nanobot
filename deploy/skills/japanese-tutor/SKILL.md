...
- 正式课程严格以已扫描生成的 `data/curriculum-n1.yaml` 为路线：**《新标准日本语》初级48课 → 中级32课 → 高级24课，共104课**。默认一次正式课程单元对应教材完整一课，按节点顺序推进，不另造一套平行基础课程。
- 每课必须使用该节点中的 `textbook` 页码、`themes`（语法／交际功能／文化主题）、`question_types` 与 `vocabulary_selector` 规划教学；AI课程以本课语法、课文理解、交际应用、听力与必要练习为完成范围，全部覆盖并验收后才能推进下一课。这份课程图是课程安排的权威来源，不只是检索资料。
- 一课内容确实过多时才明确拆成「第X课·上／下」等连续部分，并在状态中记录已覆盖与待覆盖的语法、课文／交际功能、听力和练习；下一次必须继续同一课，待覆盖清零后才能标记该课完成。不得静默省略本课教学内容。
- 假名、旅行问路、游戏日语、发音纠错、JLPT专项与主动输出薄弱点，只能作为当前教材课的快速验收、例句场景、补充练习或复习项；不得阻塞、替代或打乱《新标日》的课次顺序。
- 只有用户明确要求跳课／改线，或前置能力缺失有真实证据且无法在当前教材课内补足时，才允许调整路线；调整必须记录原因，不得凭模型感觉自行回退到桥接节点。

## Anki 课程边界

- 教材词汇由用户在 Anki 客户端独立学习和复习。正式课堂默认完全跳过 Anki 状态读取、到期复习、词卡评分、句子卡候选与写卡，不调用 Anki 接口；课程专注《新标准日本语》的语法、课文、听力和主动输出。
- 课堂材料直接依据课程节点的 `vocabulary_selector` 和教材页码控制范围，不为获取词表而默认访问 Anki。
- 只有用户当次明确要求联动 Anki 时，才按 [references/anki-workflow.md](references/anki-workflow.md) 检查真实到期卡或牌组；只处理用户点名的范围，不把 Anki 变成教材课完成门槛。
- 只有用户明确要求制作句子卡时，才提出候选；候选先预览，用户再次确认后方可调用 `anki_adapter.py add-note --confirmed` 写入。教材牌组与沉浸牌组不得重复造卡。
