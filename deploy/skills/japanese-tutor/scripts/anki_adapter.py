...
TARGET_DECKS = (*RECOGNITION_DECKS, *PRODUCTION_DECKS)
LEVEL_DECKS = dict(zip(("beginner", "intermediate", "advanced"), RECOGNITION_DECKS, strict=True))
...
def deck_level(deck: str) -> str | None:
    for decks in (RECOGNITION_DECKS, PRODUCTION_DECKS):
        match = next(
            (level for level, name in zip(("beginner", "intermediate", "advanced"), decks, strict=True) if name == deck),
            None,
        )
        if match:
            return match
...
def due(client: AnkiClient, limit: int) -> dict[str, Any]:
    ...
    card_ids: list[int] = []
    for deck in (*TARGET_DECKS, IMMERSION_DECK):
...
def snapshot(client: AnkiClient, limit: int) -> dict[str, Any]:
    """生成 curriculum_state 可直接消费的轻量 Anki 状态。"""
    health_result = health(client)
    due_result = due(client, limit)
    deck_result = discover(client)
    counts: dict[str, dict[str, int]] = {}
    for deck in (*TARGET_DECKS, IMMERSION_DECK):
        total = client.call("findCards", {"query": f'deck:"{quote_query(deck)}"'})
        new = client.call("findCards", {"query": f'deck:"{quote_query(deck)}" is:new'})
        if not isinstance(total, list) or not isinstance(new, list):
            raise AdapterError("malformed_response", f"findCards 牌组统计响应无效: {deck}")
        counts[deck] = {"total": len(total), "new": len(new)}
    return {
        "available": bool(health_result.get("ok")),
        "reason": "no_due_cards" if due_result["total_due"] == 0 else None,
        "weaknesses": [],
        "due_cards": due_result["cards"],
        "total_due": due_result["total_due"],
        "decks": deck_result["decks"],
        "counts": counts,
    }
...
def main() -> int:
    ...
    try:
        ...
        if args.action == "health":
            ...
            result = snapshot(client, args.limit)
