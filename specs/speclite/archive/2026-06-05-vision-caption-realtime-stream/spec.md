...
- ~~本次不做：Telegram/Discord 等非 WebSocket channel 的 caption UI。~~ **已扩展**：`unifiedSession` 下外部 channel 带图入站也流式 fan-out 到 `inbox:unified`。
...
- [x] `loop.py` `_state_caption`：WebSocket + `_wants_stream`，或 `unifiedSession` + 外部 channel 时注册回调；`publish_outbound` 带 `_vision_caption_delta` / `_vision_caption_end` + `image_index` + `stream_id`；caption 完成后仍写 sentinel 到 `ctx.msg.content`（session 持久化不变）。
- [x] `channels/base.py` + `websocket.py` + `manager.py`：`send_vision_caption_delta/end`，`_send_once` 路由；`manager` 将 caption / `turn_end` fan-out 到 `inbox:unified`。
- [x] `session/webui_turns.py`：统一会话下外部 channel turn 完成也发布 `_turn_end`（供 inbox fan-out）。
...
## Apply Notes
- 流式触发条件：WebSocket + `_wants_stream`；或 `unifiedSession` + 外部 channel（Telegram 等）。CLI/无统一会话时外部 channel 仍阻塞 caption、无 WS。
...
- **unified inbox fan-out**：外部 channel 的 `vision_caption_*` 与 `turn_end` 经 `ChannelManager._maybe_fan_out_unified_inbox` 推到 `inbox:unified`；Electron 收到外部 `user`（带 `media_urls`）后重置 caption 累积器。
- caption 仍 patch **最后一条 user 消息**（Electron 中途本地发消息可能贴错——已知限制，暂不修）。
...
- [x] `pytest tests/agent/test_vision_caption.py tests/channels/test_websocket_channel.py tests/channels/test_channel_manager_unified_inbox.py tests/session/test_webui_turns_unified.py -q` 通过。
- [x] Electron：`use-nanobot-stream-inbox-user.test.tsx` 覆盖外部 user + caption 流式 + turn_end。
