...
export function SelectContent({ children, className, ...props }: Pick<
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Content>,
  "children" | "className" | "onPointerUpCapture" | "onPointerDownOutside" | "onKeyDownCapture" | "onEscapeKeyDown"
>) {
  const container = useFloatingPortal();
  return (
    <SelectPrimitive.Portal container={container ?? undefined}>
      <SelectPrimitive.Content {...props} position="item-aligned" className={cn("z-50 max-h-[min(24rem,calc(100dvh-2rem))] min-w-[10rem] overflow-hidden rounded-control border border-transparent", floatingSurfaceElevationClassName, className)}>
